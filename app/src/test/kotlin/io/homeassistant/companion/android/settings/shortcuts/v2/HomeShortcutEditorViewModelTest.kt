...
class HomeShortcutEditorViewModelTest {
    ...
    fun setup() {
        ...
        coEvery { shortcutsRepository.loadHomeEditor(homeShortcutDraft.id) } returns ShortcutResult.Success(
            HomeEditorData(draftSeed = homeShortcutDraft, mode = EditorMode.EDIT),
        )
    ...
    fun `Given home shortcut exists when openEditHomeShortcut then editor is HomeEdit with correct id`() = runTest {
        ...
        val editor = viewModel.uiState.value.editor as ShortcutEditorUiState.HomeEditorState
    ...
    fun `Given home editor load error when openEditHomeShortcut then screen error is set`() = runTest {
        coEvery { shortcutsRepository.loadHomeEditor(homeShortcutDraft.id) } returns ShortcutResult.Error(
